...
def test_create_new_project(app: Application, login):
    ...
    expect(app.page.get_by_role("heading", name=project_name)).to_be_hidden()
...
def test_switch_back_to_grid_view(app: Application, login):
    ...
    expect(app.projects_page.header.grid_view_button).to_have_class(
        "tablinks active_list_type"
    )
...
def test_non_demo_card_has_no_demo_badge(logged_app: Application):
    card = logged_app.projects_page.get_project_by_name("Gaines Group")
...
def test_projects_page_header(logged_app: Application):
    ...
    target_project = logged_app.projects_page.get_project_by_name(target_project_name)
